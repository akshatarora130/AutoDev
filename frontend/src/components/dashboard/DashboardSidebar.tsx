...
export const DashboardSidebar = ({
  ...
  return (
    <div className="relative h-full">
      <aside
        className={`bg-surface border-r border-white/5 flex flex-col shrink-0 z-10 transition-all duration-300 ease-in-out overflow-hidden h-full ${
          isOpen ? "w-[280px] opacity-100" : "w-0 opacity-0 border-r-0"
        }`}
      >
        <div className="p-4 border-b border-white/5 w-[280px]">
        ...
        <div className="flex-1 overflow-y-auto py-2 custom-scrollbar w-[280px]">
          <div className="px-2 space-y-0.5">
            ...
            {displayedRepos.length === 0 && (
              <div className="text-center py-8 px-4">
                <p className="text-xs text-text-muted mb-2">No projects found.</p>
                <Button
                  variant="link"
                  size="sm"
                  onClick={onCreateClick}
        ...
        <div className="p-4 border-t border-white/5 text-[10px] text-text-muted/40 font-mono text-center w-[280px]">
          AutoDev v1.0.0
        </div>
      </aside>
    </div>
